...
pub fn save_foreground_window(state: &PlatformState) -> Result<(), MisterTError> {
    let output = Command::new("osascript")
        ...
        .output()
        .map_err(|e| MisterTError::Injection(format!("osascript failed: {e}")))?;

    let app_name = String::from_utf8_lossy(&output.stdout).trim().to_string();

    if app_name.is_empty() {
        return Err(MisterTError::Injection("No frontmost app found".into()));
    }
    ...
    match saved.as_ref() {
        Some(WindowHandle::MacOS(app_name)) => {
            Command::new("osascript")
                ...
                .output()
                .map_err(|e| {
                    MisterTError::Injection(format!("Failed to restore window: {e}"))
                })?;
            Ok(())
        }
